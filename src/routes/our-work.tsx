...
export const Route = createFileRoute('/our-work')({
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Recent websites designed and built by DD Web Solutions for local UK businesses.' },
      { name: 'robots', content: 'index, follow' },
      { property: 'og:title', content: 'Our Work | DD Web Solutions' },
      { property: 'og:description', content: 'Recent websites designed and built by DD Web Solutions for local UK businesses.' },
      { property: 'og:type', content: 'website' },
    ],
    links: [{ rel: 'canonical', href: 'https://ddwebsolution.com/our-work' }],
  }),
})
...
function ArrowUpRight() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
      <path d="M5 13L13 5M7 5h6v6" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
export default function OurWorkPage() {
  return (
    <div style={{ fontFamily: 'var(--font-body)', color: '#0a1628' }}>
      <header className="fixed top-0 left-0 right-0 z-50" style={{ background: 'rgba(10,22,40,0.96)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <main>
        <section className="pt-36 pb-20" style={{ background: 'linear-gradient(180deg, #f0f7ff 0%, #f8fbff 55%, #ffffff 100%)' }}>
          <div className="max-w-6xl mx-auto px-5 sm:px-8">
                    ...
                    <div className="p-8 sm:p-12 flex flex-col justify-center">
                      ...
                      <div className="flex flex-wrap gap-2.5 mt-7">
                        {project.tags.map((tag) => (
                          <span key={tag} className="rounded-full px-3 py-1.5 text-xs font-semibold" style={{ background: '#e0ecf9', color: '#1e4a8a' }}>{tag}</span>
                        ))}
                      </div>
                      <div className="mt-9 flex flex-wrap gap-4 items-center">
                        <a href={project.url} target="_blank" rel="noreferrer" className="btn-primary inline-flex items-center gap-2" style={{ fontFamily: 'var(--font-body)', textDecoration: 'none' }}>View Live Website <ArrowUpRight /></a>
                        <a href="/#contact" className="font-semibold text-sm" style={{ color: '#1e4a8a', textDecoration: 'none' }}>Start your project →</a>
                      </div>
                    </div>
                  </div>
                </article>
        ...
        <section className="py-24" style={{ background: '#0a1628' }}>
          <div className="max-w-4xl mx-auto px-5 sm:px-8 text-center">
            <span className="section-label" style={{ color: '#7aaada' }}>Your next project</span>
            <h2 className="font-display mt-4 text-white" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, lineHeight: 1.12 }}>Ready to put your business online properly?</h2>
            <p className="mt-5 mx-auto" style={{ color: 'rgba(255,255,255,0.65)', maxWidth: '650px', lineHeight: 1.7 }}>Tell us what your business does, what is not working with your current website, and what you want it to achieve.</p>
            <a href="/#contact" className="btn-primary inline-block mt-8 text-sm" style={{ textDecoration: 'none' }}>Book a Free Call</a>
          </div>
        </section>
      </main>
      ...
      <footer style={{ background: '#060f20', color: 'rgba(255,255,255,0.5)' }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-8 py-10">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div><div className="font-semibold text-white text-sm">DD Web Solutions</div><p className="text-xs mt-2">Professional web design for UK small businesses.</p></div>
          ...
          <div className="border-t mt-8 pt-6 text-xs" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>&copy; {new Date().getFullYear()} DD Web Solutions. All rights reserved.</div>
        </div>
      </footer>
    </div>
  )
